...
Подробное руководство по командной утилите: [CLI — руководство и примеры](./cli/cli-usage.md)
...
```json
{
  "mcpServers": {
    "yougile": {
      "command": "python",
      "args": ["/полный/путь/к/папке/yougile-mcp/run_mcp_server.py"],
...
3. Попробуйте запустить `python run_mcp_server.py` напрямую и посмотрите на вывод

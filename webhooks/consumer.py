...
from .db import init_engine as init_webhook_engine, async_session as webhook_session_factory
    ...
    try:
        ...
        if entity_type == "task":
            ...
            if data:
                try:
                    ...
                    if "column_id" in str(ie):
                        ...
                        if column_id:
                            if await _fetch_and_create_missing_entity("column", column_id, local_session, yougile_client):
                                # Retry task creation
                                await _upsert_task_from_payload(data, local_session)
                                await local_session.commit()
                                logger.info(f"Created missing task {entity_id} after resolving column")
                                return True
                    raise
            ...
            if data:
                ...
                if project_id:
                    await _fetch_and_create_missing_entity("project", project_id, local_session, yougile_client)
            ...
            if data:
                ...
                if board_id:
                    await _fetch_and_create_missing_entity("board", board_id, local_session, yougile_client)
            ...
            try:
                stickers_list = await api_stickers.get_sprint_stickers(yougile_client)
                ...
                logger.warning(f"Sticker {entity_id} not found in company stickers list")
                return False
            except Exception as e:
...
async def _upsert_task_from_payload(payload: Dict[str, Any], local_session: AsyncSession) -> None:
...
async def _upsert_project_from_payload(payload: Dict[str, Any], local_session: AsyncSession) -> None:
...
async def _upsert_comment_from_payload(payload: Dict[str, Any], local_session: AsyncSession) -> None:
    ...
    author_id = payload.get("actionBy") or payload.get("authorId") or payload.get("userId")
    if not author_id:
        properties = payload.get("properties", {})
    ...
    comment_data = {
        ...
        "timestamp": datetime.fromtimestamp(payload["timestamp"] / 1000) if payload.get("timestamp") else datetime.utcnow(),
    }
    
    comment = await local_session.merge(Comment(**comment_data))
    logger.debug(f"Upserted comment {comment_id_str}")


async def _upsert_sticker_from_payload(payload: Dict[str, Any], local_session: AsyncSession) -> None:
    ...
    if is_sprint:
        ...
        sticker = await local_session.merge(SprintSticker(**sticker_data))
        
        # Delete existing states and recreate from payload
        await local_session.execute(delete(SprintState).where(SprintState.sticker_id == sticker_id))
        ...
        for state_data in states:
            ...
            state = SprintState(
                ...
                begin=datetime.fromtimestamp(state_data["begin"] / 1000) if state_data.get("begin") else None,
                end=datetime.fromtimestamp(state_data["end"] / 1000) if state_data.get("end") else None,
            )
            local_session.add(state)
        
        logger.debug(f"Upserted sprint sticker {sticker_id}: {sticker_name} with {len(states)} states")
    else:
        ...
        sticker = await local_session.merge(StringSticker(**sticker_data))
        
        # Delete existing states and recreate from payload
        await local_session.execute(delete(StringState).where(StringState.sticker_id == sticker_id))
        ...
        logger.debug(f"Upserted string sticker {sticker_id}: {sticker_name} with {len(states)} states")


async def _upsert_board_from_payload(payload: Dict[str, Any], local_session: AsyncSession) -> None:
...
async def _upsert_column_from_payload(payload: Dict[str, Any], local_session: AsyncSession) -> None:
...
async def _upsert_user_from_payload(payload: Dict[str, Any], local_session: AsyncSession) -> None:
...
async def _upsert_department_from_payload(payload: Dict[str, Any], local_session: AsyncSession) -> None:
...
async def catch_up(
    ...
    if local_session_factory:
        ...
        logger.info("Prefetching company-wide entities (users, stickers, departments)...")
        try:
            async with local_session_factory() as prefetch_session:
                ...
                try:
                    ...
                    sticker_count = 0
                    for sticker_data in stickers_list or []:
                        await _upsert_sticker_from_payload(sticker_data, prefetch_session)
                        sticker_count += 1
                    ...
                    if local_session_factory and entity_data:
                        async with local_session_factory() as local_sess:
                            try:
                                ...
                                entity_prefix = event_type.split("-")[0] if "-" in event_type else entity_type
                                ...
                                if entity_prefix == "task":
                                    ...
                                    await _upsert_department_from_payload(entity_data, local_sess)
                                elif entity_prefix == "group_chat":
                                    # No model for group_chat yet, just log
                                    logger.debug(f"Skipping group_chat event (no model): {entity_data.get('id')}")
                                else:
                                ...
                                if "foreign key" in error_msg.lower() or "violates foreign key constraint" in error_msg.lower():
                                    logger.warning(f"FK constraint error for event #{event.id}, attempting to resolve")
                                    ...
                                    if "task_id" in error_msg:
                                        ...
                                        missing_id = entity_data.get("taskId") or entity_data.get("chatId")
                                        if not missing_id:
                                        ...
                                        fk_field = "user"
                                        missing_id = entity_data.get("authorId") or entity_data.get("userId")
                                    
                                    # Try to fetch and create missing entity
                                    if fk_field and missing_id and yougile_client:
                                        resolved = await _fetch_and_create_missing_entity(
                                            fk_field, missing_id, local_sess, yougile_client
                                        )
                                        ...
                                        if resolved:
                                            ...
                                            try:
                                                ...
                                                await local_sess.commit()
                                                fk_resolved_count += 1
                                                logger.info(f"Successfully resolved FK error for event #{event.id}")
                                            except Exception as retry_err:
                                                logger.error(f"Retry failed for event #{event.id}: {retry_err}")
                                                await local_sess.rollback()
                                                raise
                                        else:
                                            logger.error(f"Could not resolve FK error for event #{event.id}")
                                            raise
                                    else:
                                        logger.error(f"Cannot auto-resolve FK error for event #{event.id}: field={fk_field}, id={missing_id}")
                                        raise
                                else:
                                    # Not a FK error, re-raise
                                    raise
                            except Exception as proc_err:
                                logger.error(f"Failed to process entity for event #{event.id}: {proc_err}")
                                await local_sess.rollback()
                                raise
...
async def process_single_event(event_id: int, local_db_url: Optional[str] = None) -> bool:
    """
    ...
    # Get local DB URL
    if not local_db_url:
        local_db_url = os.environ.get("YOUGILE_LOCAL_DB_URL") or settings.yougile_local_db_url
            ...
            if entity_data and local_session_factory:
                async with local_session_factory() as local_sess:
                    try:
                        ...
                        entity_prefix = event_type.split("-")[0] if "-" in event_type else (event.entity_type or "unknown")
